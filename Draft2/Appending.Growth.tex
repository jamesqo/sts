...
Now, consider the ratio $\lim_{n \to \infty} \dfrac {\FuncSpaceTail(\VarUseful)} {\VarCapacitySeq_\VarUseful} = \dfrac {O(\log n)} {O(n)} = O \left( \dfrac{\log n}{n} \right) = 0$. Because it is $0$, from Theorem \ref{thm:RemovesLowerOrderTerms} we may conclude
\begin{align*}
...
Using Lemma \ref{lem:VarUsefulIsOLogN} again, $\FuncSpaceTail(\VarUseful) = O(\VarUseful) = O(\log n)$. Similarly to before, the $\FuncSpaceTail(\VarUseful)$ term vanishes under $\sim$:
\begin{align*}
\FuncSpace(n) \sim ... + O(\log n) = ...
\end{align*}