...
I now approximate this expression using $\TextTilde$. First, I approximate $k$ using big-O:
...
Now, the $k$-term disappears when using $\TextTilde$ to approximate $\FuncWritesNew(n)$, leaving just $n$:

\begin{align*}
\FuncWritesNew(n) &\TextTilde n + \FuncWrites(k)\\
&= n + O(\log n)\\
&\TextTilde n
\end{align*}
...
\begin{align*}
\FuncSpaceNew(n) \TextTilde 2^{\lceil \log_2 n \rceil} = O(n)
\end{align*}