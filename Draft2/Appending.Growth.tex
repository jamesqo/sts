...
\begin{algorithm}
	\begin{algorithmic}[1]
		...
		\If{$\VarList.\FieldHeadCapacity = \VarInitCapacity$}
			\State $new\ hcap \gets (\VarGrowthFactor - 1) \times \VarList.\FieldHeadCapacity$
		\Else
			\State $new\ hcap \gets \VarGrowthFactor \times \VarList.\FieldHeadCapacity$
		\EndIf
		\State $\VarList.\FieldHead \gets \FuncNewArray(new\ hcap)$
		\State $\VarList.\FieldCapacity \gets \VarList.\FieldCapacity + new\ hcap$
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
